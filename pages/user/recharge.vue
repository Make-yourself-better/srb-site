            ...
            <div class="fl quick-info">
              <div class="info-1">
                ...
                <span class="info1-input">
                  <input type="text" class="pay-money-txt" maxlength="10" v-model="chargeAmt" />
                  <em>元</em>
                </span>
              </div>
              ...
              <input type="button" value="充值" class="btn-paycz" @click="commitCharge()" />
            </div>
        ...
        '前往汇付宝资金托管平台',
        {
          ...
          callback: (action) => {
            if (action === 'confirm') {
              this.$axios
                .$post(
                  '/admin/core/userAccount/auth/commitCharge/' +
                    this.chargeAmt
                )
                .then((response) => {
