  ...
  <div class="personal-main">
    <div class="personal-pay">
      ...
      <div v-if="active === 0" class="user-borrower">
        <el-form label-width="120px">
          ...
          <el-form-item label="还款方式">
            <el-select v-model="borrowInfo.returnMethod">
              <el-option v-for="item in returnMethodList" :key="item.value" :label="item.name" :value="item.value" />
            </el-select>
          </el-form-item>
          <el-form-item label="资金用途">
            <el-select v-model="borrowInfo.moneyUse">
              <el-option v-for="item in moneyUseList" :key="item.value" :label="item.name" :value="item.value" />
            </el-select>
          </el-form-item>
        ...
        <el-alert title="您提供的任何信息尚融宝都承诺予以保护，不会挪作他用。" type="warning" :closable="false">
        </el-alert>
      </div>

      <div v-if="active === 1">
        <div style="margin-top:40px;">
          <el-alert title="您的借款申请已成功提交，请耐心等待" type="warning" show-icon :closable="false">
      ...
      <div v-if="active === 2">
        <div style="margin-top:40px;">
          <el-alert v-if="borrowInfoStatus === 2" title="您的借款申请审批已通过" type="success" show-icon :closable="false">
          </el-alert>

          <el-alert v-if="borrowInfoStatus === -1" title="您的借款申请审批未通过" type="error" show-icon :closable="false">
          </el-alert>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      this.$axios
        .$post('/admin/core/borrowinfo/auth/save', this.borrowInfo)
        .then((response) => {
          this.active = 1
        })
    },
    ...
    initSelected() {
      //还款方式列表
      this.$axios
        .$get('/admin/core/dict/findByDictCode/returnMethod')
        .then((response) => {
          this.returnMethodList = response.data.dictList
        }),
        //资金用途列表
        this.$axios
          .$get('/admin/core/dict/findByDictCode/moneyUse')
          .then((response) => {
    ...
    getBorrowAmount() {
      this.$axios
        .$get('/admin/core/borrowinfo/auth/getBorrowAmount')
        .then((response) => {
    ...
    getBorrowInfoStatus() {
      this.$axios
        .$get('/admin/core/borrowinfo/auth/getBorrowInfoStatus')
        .then((response) => {
