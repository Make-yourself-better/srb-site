...
export default {
  data() {
    ...
    return {
      ...
      uploadUrl: BASE_API + '/api/ali/fileoss/upload', //文件上传地址
  ...
  methods: {
    save() {
      ...
      this.$axios
        .$post('/admin/core/borrower/saveBorrower', this.borrower)
        .then((response) => {
          this.active = 1
        })
    },
    ...
    onUploadRemove(file, fileList) {
      ...
      //删除oss服务器上的内容
      this.$axios
        .$delete('api/ali/fileoss/removeFile?url=' + file.response.data.url)
        .then((response) => {
          // debugger
          console.log('远程删除')
    ...
    getUserInfo() {
      this.$axios.$get('/admin/core/borrower/auth/getBorrowerStatus').then(res => {
