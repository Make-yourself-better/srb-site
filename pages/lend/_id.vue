      ...
      <div class="bd clearfix">
        <div class="data" style="width: auto;">
          <ul>
            ...
            <li>
              <span class="c-888">还款方式：</span>{{ lend.map.returnMethod }}
            </li>
          ...
          <div class="borrow-info" style="width:auto;">
            ...
            <dl class="item">
              ...
              <dd>
                <div class="text">
                  <p class="MsoNormal" style="margin-left:0cm;text-indent:0cm;">
                    姓名：{{ borrower.borrower.name }} <br />
                    手机号码：{{ borrower.borrower.mobile }} <br />
                    身份认证：已认证 <br />
                    注册时间：{{ borrower.borrower.createTime }} <br />
                  </p>
                </div>
              </dd>
            </dl>
...
export default {
  async asyncData({ $axios, params }) {
    ...
    //通过lendId获取标的详情信息
    let response = await $axios.$get('/admin/core/lend/show/' + lendId)
  ...
  methods: {
    ...
    fetchAccount() {
      ...
      if (userInfo) {
        this.$axios
          .$get('/admin/core/userAccount/auth/getAccount')
          .then((response) => {
    ...
    getInterestCount() {
      this.$axios
        .$get(
          `/admin/core/lend/getInterestCount/${this.invest.investAmount}/${this.lend.lendYearRate}/${this.lend.period}/${this.lend.returnMethod}`
        )
        .then((response) => {
        ...
        '前往汇付宝资金托管平台',
        {
          ...
          callback: (action) => {
            ...
            if (action === 'confirm') {
              ...
              this.$axios
                .$post('/admin/core/lendItem/auth/commitInvest', this.invest)
                .then((response) => {
